...
(function() {
  ...
  runner = function() {
    var debug, exclusions, extraExts, exts, fs, livereload, options, opts,  path, pjson, port, resolve, server, usePolling, version, wait;
    fs = require('fs');
    ...
    if(opts.get('https')) {
      ...
      options.https.key = fs.readFileSync(opts.get('key') ? opts.get('key') : '/cert/local.key');
      options.https.cert = fs.readFileSync(opts.get('cert') ? opts.get('cert') : '/cert/local.crt');
